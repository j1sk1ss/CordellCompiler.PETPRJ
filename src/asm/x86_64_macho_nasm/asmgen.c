...
static int _convert_lirblock_to_assembly(lir_block_t* b, func_info_t* fi, sym_table_t* smt, FILE* output) {
    ...
    switch (b->op) {
        ...
        case LIR_BREAKPOINT: if (CONF_is_debug_compilation()) EMIT_COMMAND("int3 ; %s", x86_64_macho_nasm_format_lir_subject(b->farg, smt, NO_FLAG));                                    break;
        ...
        case LIR_RAW: {
            ...
            if (percent_pos < 0) EMIT_COMMAND("%s", raw_line->body);
                ...
                string_t* replacement = create_string(x86_64_macho_nasm_format_lir_subject(b->sarg, smt, NO_FLAG));
