    ...
    iterate_lir_instructions (bb) {
        switch (lh->op) {
            case LIR_STSARG: {
                ...
                lir_subject_t* nfarg = x86_64_gnu_nasm_create_tmp(sys_regs[lh->sarg->storage.cnst.value], lh->farg, smt, CONF_get_full_bytness());
            ...
            case LIR_STARGLD: {
                ...
                switch (lh->sarg->storage.cnst.value) {
                    case 0: {
                        src    = fi->flags.abi ? x86_64_gnu_nasm_create_tmp(RDI, lh->farg, smt, -1) : LIR_SUBJ_OFF(RBP, -8, CONF_get_full_bytness());
                        lh->op = LIR_iMOV; 
                        break;
                    }
                    default: {
                        src    = fi->flags.abi ? x86_64_gnu_nasm_create_tmp(RSI, lh->farg, smt, -1) : LIR_SUBJ_OFF(RBP, -16, CONF_get_full_bytness());
            ...
            case LIR_LOADFRET: {
                lh->op   = LIR_iMOV;
                lh->sarg = x86_64_gnu_nasm_create_tmp(RAX, lh->farg, smt, -1);
                break;
            }
            case LIR_NOT: {
            ...
            case LIR_iDIV:
            case LIR_iMOD: {
                lir_subject_t* a_entry = x86_64_gnu_nasm_create_tmp(RAX, lh->sarg, smt, CONF_get_full_bytness());
                _insert_instruction_before(bb, LIR_create_block(LIR_iMOV, a_entry, lh->sarg, NULL), lh);

                lir_subject_t* oldres = lh->farg;
                lh->sarg = a_entry;

                lir_subject_t* b = x86_64_gnu_nasm_create_tmp(RCX, lh->targ, smt, CONF_get_full_bytness());
                _insert_instruction_before(bb, LIR_create_block(LIR_iMOV, b, lh->targ, NULL), lh);
                lh->targ = b;

                lir_subject_t* mod = x86_64_gnu_nasm_create_tmp(RDX, lh->farg, smt, CONF_get_full_bytness());
            ...
            case LIR_iCMP: case LIR_iNMP: {
                ...
                switch (lh->op) {
                    ...
                    default: {
                        if (x86_64_gnu_nasm_is_sign_type(lh->sarg, smt) && x86_64_gnu_nasm_is_sign_type(lh->targ, smt)) {
                            switch (lh->op) {
            ...
            case LIR_TU64: case LIR_TU32: case LIR_TU16: case LIR_TU8: {
                lh->op = x86_64_gnu_nasm_get_proper_mov(lh->farg, lh->sarg, smt, LIR_iMOV);
                break;
            }
            default: break;
        }
